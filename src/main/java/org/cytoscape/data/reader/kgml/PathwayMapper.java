...
//					if (!titleNodePattern.matcher(grap.getName()).matches()) {
                    if (!grap.getType().equals(KEGGShape.LINE.getTag())) {
                        ...
                        nodeAttr.setListAttribute(node.getIdentifier(), KEGG_NAME_LIST, java.util.Arrays.asList(comp.getName().split(" ")));
                        ...
                        if (reaction != null) {
                            ...
                            nodeAttr.setListAttribute(node.getIdentifier(), KEGG_REACTION_LIST, java.util.Arrays.asList(reaction.split(" ")));
                        }
                        ...
                        } else if (comp.getType().equals(KEGGEntryType.GENE.getTag()) || comp.getType().equals(KEGGEntryType.ORTHOLOG.getTag())) {
                ...
                if (rel.getType().equals(KEGGRelationType.MAPLINK.getTag())) {
                    ...
                    if (entryMap.get(rel.getEntry1()).getType().equals(KEGGEntryType.MAP.getTag())) {
                        ...
                        for (Subtype sub : subs) {
                            ...
                            CyEdge edge2 = Cytoscape.getCyEdge(maplinkNode, cpdNode, Semantics.INTERACTION, type, true, true);
                            edges.add(edge2);
...
//					edgeAttr.setAttribute(edge.getIdentifier(), KEGG_RELATION, type);
    ...
    protected void updateView(final CyNetwork network) {
        ...
        if (pathway_entryID.equals(METABOLIC_PATHWAYS_ENTRY_ID) || pathway_entryID.equals(BIOSYNTHESIS_OF_SECONDARY_METABOLITES_ENTRY_ID)) {
            ...
            final DiscreteMapping edgeWidthMap = new DiscreteMapping(3, Semantics.INTERACTION, ObjectMapping.EDGE_MAPPING);
            final Calculator edgeWidthCalc = new BasicCalculator(vsName + "-" + "EdgeWidthMapping", edgeWidthMap, VisualPropertyType.EDGE_LINE_WIDTH);
            ...
            final DiscreteMapping nodeColorMap = new DiscreteMapping(NODE_DEF_COLOR, KEGG_ENTRY, ObjectMapping.NODE_MAPPING);
            final Calculator nodeColorCalc = new BasicCalculator(vsName + "-" + "NodeColorMapping", nodeColorMap, VisualPropertyType.NODE_FILL_COLOR);
        ...
        final Calculator nodeWidthCalc = new BasicCalculator(vsName + "-" + "NodeWidthMapping", nodeWidth, VisualPropertyType.NODE_WIDTH);
        final DiscreteMapping nodeHeight = new DiscreteMapping(30, "ID", ObjectMapping.NODE_MAPPING);
        final Calculator nodeHeightCalc = new BasicCalculator(vsName + "-" + "NodeHeightMapping", nodeHeight, VisualPropertyType.NODE_HEIGHT);
